...
"""
-------------------------------------------------
   File Name：     down_service.py
   Description :   service of down url
   Author :       LHY
   date：          2020/09/03
-------------------------------------------------
   Change Activity:
...
class pythonServiceDemo(win32serviceutil.ServiceFramework):
    # Service name
    _svc_name_ = "DownService"
    # Service name in win
    _svc_display_name_ = "Down Service"
    # Service description
    _svc_description_ = "This service provide main download execution service!"
    ...
    def _getLogger(self):
        logger = logging.getLogger('[Down Service Log]')

        handler = logging.FileHandler(os.path.abspath(os.path.join(log_path, "down_service.log")))
    ...
    def SvcStop(self):
        self.logger.info("down service is stop ...")
...
class myService(pythonServiceDemo):
    def start(self):
        """ start service"""
        self.child = subprocess.Popen("python proxyPool.py downUrl", cwd=rot_path)
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        try:
            ...
            servicemanager.Initialize('DownService', evtsrc_dll)
