...
class pythonServiceDemo(win32serviceutil.ServiceFramework):
    ...
    # Service name in win
    _svc_display_name_ = "Proxy Service"
    # Service description
    _svc_description_ = "This service provide proxy pool service!"
    ...
    def _getLogger(self):
        logger = logging.getLogger('[Proxy Service Log]')

        handler = logging.FileHandler(os.path.abspath(os.path.join(log_path, "proxy_service.log")))
    ...
    def SvcDoRun(self):
        self.logger.info("proxy service is run ...")
        self.start()
        win32event.WaitForSingleObject(self.hWaitStop, win32event.INFINITE)

    def SvcStop(self):
        self.logger.info("proxy service is stop ...")
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        try:
            ...
            servicemanager.Initialize('ProxyService', evtsrc_dll)
