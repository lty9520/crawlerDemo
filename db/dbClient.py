...
class DbClient(object):
    ...
    抽象方法定义：
        get(): 随机返回一个proxy;
        getdownUrl(): 随机返回一个downUrl
        put(proxy): 存入一个proxy;
        putUrl():   存入一个downUrl
        pop(): 顺序返回并删除一个proxy;
        popUrl():  顺序返回并删除一个downUrl
        update(proxy): 更新指定proxy信息;
        updateUrl(downurl):  更新指定downUrl信息
        delete(proxy): 删除指定proxy;
        deleteUrl(downurl):  删除指定downUrl
        exists(proxy): 判断指定proxy是否存在;
        existsUrl(downurl):  判断指定downUrl是否存在
        getAll(): 返回所有代理;
        clean(): 清除所有proxy信息;
    ...
    def getdownUrl(self, **kwargs):
        return self.client.get(**kwargs)

    def put(self, key, **kwargs):
        return self.client.put(key, **kwargs)

    def putUrl(self, key, **kwargs):
        return self.client.put(key, **kwargs)

    def update(self, key, value, **kwargs):
        return self.client.update(key, value, **kwargs)

    def updateUrl(self, key, value, **kwargs):
        return self.client.update(key, value, **kwargs)

    def delete(self, key, **kwargs):
        return self.client.delete(key, **kwargs)

    def deleteUrl(self, key, **kwargs):
        return self.client.delete(key, **kwargs)

    def exists(self, key, **kwargs):
        return self.client.exists(key, **kwargs)

    def existsUrl(self, key, **kwargs):
        return self.client.exists(key, **kwargs)

    def pop(self, **kwargs):
        return self.client.pop(**kwargs)

    def popUrl(self, **kwargs):
        return self.client.pop(**kwargs)
